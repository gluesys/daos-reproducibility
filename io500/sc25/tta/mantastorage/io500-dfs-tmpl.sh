...
function setup(){
  ...
  [ -f config-all.ini ] && mv config-all.ini config-all.ini.bak
  cp -af config-all-dfs-tmpl-rf0.ini config-all.ini

  sed -i -e "s/@DAOS_POOL/$DAOS_POOL/g" config-all.ini
  sed -i -e "s/@DAOS_CONT/$DAOS_CONT/g" config-all.ini
  sed -i -e "s/@FIND_NPROC/$FIND_NPROC/g" config-all.ini
  sed -i -e "s/@FIND_QLEN/$FIND_QLEN/g" config-all.ini
}
